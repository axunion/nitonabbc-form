...
**トークン契約**: 共有フォームコンポーネント（`src/components/forms/`）の `.module.css` は `--color-*` / `--space-*` / `--text-*` / `--radius-*` / `--shadow-*` トークンを前提とする。共有コンポーネントを使うページは必ずいずれかのテーマを import すること。新規テーマを作成する際は、既存テーマ（`indigo.css`）が定義するトークン一式をすべて定義すること。
